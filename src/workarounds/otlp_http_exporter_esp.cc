        ...
        &spans) noexcept
{
  ...
  // ESP-IDF malloc guarantees only 4-byte alignment, but TaggedAllocationPolicyPtr uses bits 0-2
  // as tag bits (kTagsMask=7), requiring 8-byte aligned pointers. Without this pre-allocated
  // buffer, get() strips bit 2 from the misaligned AllocationPolicy pointer, reading max_block_size
  // as block_alloc and calling 0x10000 as a function pointer -> InstrFetchProhibited crash.
