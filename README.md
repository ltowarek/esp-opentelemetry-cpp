...
| ESP log | `CONFIG_ESP_OPENTELEMETRY_EXPORTER_ESP_LOG` | `esp_opentelemetry::MakeEspLog*Exporter()` — each item as one ordinary ESP-IDF log line (`otel.span`/`otel.log`/`otel.metric`/`otel.profile` tags), filtered and shown like any other component's output; runs under QEMU |
...
| [`examples/logs/`](examples/logs/) | One signal, simplest exporter: the ESP log exporter printing log records, including the level and tag filtering rules in action. | None (QEMU) |
...
| `include/esp_log_exporters.hpp` / `src/integration/esp_log_exporters.cpp` | `MakeEspLog*Exporter()` — the component's own formatter (no C++ streams, no wrapping the Ostream exporters) that prints each span, log record, metric data point and profile as one `ESP_LOGx` line under `otel.span`/`otel.log`/`otel.metric`/`otel.profile`, skipping an item `esp_log` itself would not print before formatting it |
...
|---------|--------|---------|
| `OStreamSpanExporter` | Not tested | — |
...
| `OStreamMetricExporter` | Not tested | — |
...
| `OStreamLogRecordExporter` | Not tested | — |
