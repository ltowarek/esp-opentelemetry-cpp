...
| `include/esp_json_writer.hpp` / `src/integration/esp_json_writer_cjson.cpp` | `JsonWriter` — a token-stream JSON emitter (begin/end object/array, key, typed writes), cJSON-backed; a second, independent implementation of the interface proposed to opentelemetry-cpp (`exporters/otlp/otlp_json_writer.h` there, nlohmann-backed), proven by the profiles exporter below |
| `src/integration/esp_profiles_document.cpp` | `esp_opentelemetry::export_profiles()` — builds the OTLP profiles (`v1development`) document through `JsonWriter` and hands it to the installed `ProfilesExporter`; opentelemetry-cpp has no profiles SDK to build it for us |
